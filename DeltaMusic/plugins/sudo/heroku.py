...
async def usage_dynos(client, message, _):
    ...
    text = f"""
**Penggunaan Dynos Heroku**

<u>Penggunaan:</u>
Total digunakan: `{AppHours}`**j**  `{AppMinutes}`**m**  [`{AppPercentage}`**%**]

<u>Dynos tersisa:</u>
Total tersisa: `{hours}`**j**  `{minutes}`**m**  [`{percentage}`**%**]"""
...
async def update_(client, message, _):
    ...
    if verification == "":
        return await response.edit("Bot sudah terbaru dengan repo upstream!")
    updates = ""
    ...
    if await is_heroku():
        try:
            ...
            for x in served_chats:
                try:
                    await app.send_message(
                        x,
                        f"{config.MUSIC_BOT_NAME} baru saja memulai ulang dirinya sendiri untuk memperbarui bot. Maaf atas ketidaknyamanannya.\n\nAnda dapat mulai memutar lagi setelah 15-20 detik.",
                    )
                    ...
                    pass
            await response.edit(
                f"{nrs.text}\n\nBot berhasil diperbarui! Tunggu beberapa menit hingga bot memulai ulang dan menerapkan perubahan!"
            )
            os.system(
            ...
            await response.edit(
                f"{nrs.text}\n\nTerjadi kesalahan saat mencoba merestart bot musik, silakan periksa log untuk mengetahui apa yang salah."
            )
            return await app.send_message(
                config.LOG_GROUP_ID,
                f"Terjadi pengecualian di #updater karena: <code>{err}</code>",
            )
    else:
        served_chats = await get_active_chats()
        for x in served_chats:
            try:
                await app.send_message(
                    x,
                    f"{config.MUSIC_BOT_NAME} baru saja merestart dirinya sendiri untuk memperbarui bot. Maaf atas ketidaknyamanannya.\n\nAnda dapat mulai memutar lagi setelah 15-20 detik.",
                )
                await remove_active_chat(x)
                await remove_active_video_chat(x)
            except Exception:
                pass
        await response.edit(
            f"{nrs.text}\n\nBot berhasil diperbarui! Tunggu beberapa menit hingga bot merestart dan menerapkan perubahan!"
        )
...
async def restart_(_, message):
    ...
    try:
        ...
        pass
    await response.edit(
        "Proses restart dimulai dengan sukses, tunggu beberapa menit hingga bot restart."
    )
    os.system(f"kill -9 {os.getpid()} && bash start")
